...
INPUT_FILE = os.path.join(DATA_DIR, 'discovered_urls.json')
...
def main():
    ...
    for url_item in urls:
        url = url_item.get('url', '') # Extract the actual URL string
        if not url:
            logger.warning(f"Skipping invalid URL item: {url_item}")
            continue
